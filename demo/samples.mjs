...
// Rounded-rect / rect hit tests shared by the framed samples.
...
function inR(x, y, x0, y0, x1, y1) {
  return x >= x0 && x < x1 && y >= y0 && y < y1;
}

// Deterministic PRNG so noisy samples render identically everywhere.
...
export function createSampleSuite({ makeCanvas, makeImageData }) {
  ...
  return function build() {
    ...
    const scaled_rgb = (r, g, b, s) => [r * 255 * s, g * 255 * s, b * 255 * s];
    return [
      ...
      // Soft-alpha avatar: a feathered disc over a smooth two-axis color wash.
      makeSample('avatar.png', 256, 256, (u, v, x, y) => {
        const d = Math.hypot(x + 0.5 - 128, y + 0.5 - 128);
        const a = clamp255(((118 - d) / 8 + 0.5) * 255);
        return [90 + 130 * u, 70 + 110 * v, 200 - 120 * u * v, a];
      }),

      // Hard-edge sprite: an aliased star with an offset translucent drop shadow
      // on transparent.
      makeSample('sprite.png', 256, 256, (_u, _v, x, y) => {
        const star = (px, py) => {
          const ang = Math.atan2(py - 128, px - 128);
          const d = Math.hypot(px - 128, py - 128);
          const t = ((ang / (2 * Math.PI)) * 5 + 5) % 1;
          return d <= 48 + 60 * Math.abs(1 - 2 * t);
        };
        if (star(x, y)) return [250, 200, 40, 255];
        if (star(x - 10, y - 10)) return [20, 20, 30, 90];
        return [0, 0, 0, 0];
      }),

      // Flat-color badge: rounded-corner card of four solid bands with faint stripes.
      makeSample('badge.png', 256, 256, (u, v, x, y) => {
        const rad = 46;
        const dx = Math.max(rad - x, x - 255 + rad, 0);
        const dy = Math.max(rad - y, y - 255 + rad, 0);
        const a = clamp255((rad - Math.hypot(dx, dy) + 0.5) * 255);
        const bands = [[235, 64, 52], [255, 255, 255], [52, 120, 235], [46, 174, 82]];
        const c = bands[Math.min(3, Math.floor(v * 4))];
        const stripe = Math.floor(u * 8) % 2 === 0 ? 0 : 12;
        return [c[0] - stripe, c[1] - stripe, c[2] - stripe, a];
      }),

      // Opaque photo-like plasma: layered sines plus seeded per-pixel noise.
      makeSample('photo.png', 256, 256, (() => {
        const rng = mulberry32(0xc0ffee);
        return (u, v) => {
          const base = Math.sin(u * 6.1 + Math.sin(v * 4.7) * 2.0) + Math.sin(v * 5.3 + Math.sin(u * 3.1) * 1.5);
          const detail = Math.sin(u * 61) * Math.sin(v * 53) * 0.25;
          const n = (rng() - 0.5) * 0.12;
          return [
            (0.5 + 0.35 * Math.sin(base + detail + n)) * 255,
            (0.5 + 0.35 * Math.sin(base * 1.3 + 1.9 + n)) * 255,
            (0.5 + 0.35 * Math.sin(base * 0.7 + 4.0 + detail)) * 255,
          ];
        };
      })()),

      // Hue blend under a left-to-right 0..255 alpha sweep - makes alpha
      // level-reduction banding easy to eyeball.
      makeSample('alpha-ramp.png', 256, 128, (u, v) =>
        [255 * (1 - u), 160 * v + 60, 255 * u, u * 255]),
      ...
      makeSample('frame.png', 348, 423, (_u, _v, x, y) => {
        if (!inRR(x, y, 23, 23, 325, 333, 10)) return [251, 251, 251]; // body 302x310
        const teal = [84, 196, 200], purple = [150, 40, 146];
        if (inR(x, y, 31, 31, 170, 174) || inR(x, y, 178, 182, 317, 325)) return teal; // cells 139x143
        if (inR(x, y, 178, 31, 317, 174) || inR(x, y, 31, 182, 170, 325)) return purple;
        return [0, 0, 0];
      }),
      ...
      makeSample('gray-squares.png', 348, 423, (_u, _v, x, y) => {
        if (!inRR(x, y, 23, 23, 325, 333, 10)) return [251, 251, 251];
        if (inR(x, y, 66, 66, 135, 139)) return [255, 255, 255];
        if (inR(x, y, 213, 66, 282, 139)) return [140, 140, 140];
        if (inR(x, y, 66, 217, 135, 290)) return [85, 85, 85];
        if (inR(x, y, 213, 217, 282, 290)) return [255, 255, 255];
        return [0, 0, 0];
      }),
