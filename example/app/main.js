...
// A real consumer would `import ... from '@graysonlang/finehash'`; the example
// imports the local TypeScript source directly so it always tracks src/.
import * as finehash from '../../src/index';
...
const CELL = 140; // longest preview side, px - keep in sync with --cell in index.html
...
function downsampleGrid(full, maxSide) {
  const g = finehash.downsampleToWorkingGrid(full.data, full.width, full.height, maxSide);
...
function cell(label, node, meta, { pending = false, hash = null, bytes = null, originalBytes = null } = {}) {
...
function originalCell(img, full, dw, dh, originalBytes) {
  ...
  return cell('Original', canvas, meta, { bytes: originalBytes, originalBytes });
}
...
function blurhashCell(work, dw, dh, originalBytes) {
  ...
  return cell('BlurHash', peekable(rgbaCanvas(pixels, dw, dh, dw, dh)), `<b>${hash.length}</b> chars &middot; ${cx}&times;${cy}`, { hash, bytes: hash.length, originalBytes });
}
...
function thumbhashCell(work, dw, dh, originalBytes) {
  ...
  return cell('ThumbHash', peekable(rgbaCanvas(rgba, w, h, dw, dh)), `<b>${hash.length}</b> B &middot; ${base64.length} chars`, { hash: base64, bytes: hash.length, originalBytes });
}
...
async function teenyWebpCell(img, dw, dh, originalBytes) {
  ...
  if (!webp) {
    return cell('Teeny WebP', out, '<span class="badge">no webp</span>');
  }
  ...
  return cell('Teeny WebP', peekable(out), `<b>${bytes}</b> B${tag} &middot; ${b64.length} chars`, { hash: b64, bytes, originalBytes });
}
...
function finehashCell(work, dw, dh, originalBytes) {
  ...
  try {
    ...
    const { pixels, width: fw, height: fh } = finehash.decode(bytes, { width: dw, height: dh });
    const meta = `<b>${bytes.length}</b> B &middot; ${b64.length} chars`;
    const canvas = peekable(rgbaCanvas(pixels, fw, fh, dw, dh));
    return cell('FineHash', canvas, meta, { hash: b64, bytes: bytes.length, originalBytes });
  } catch (err) {
    const placeholder = el('canvas', { width: dw, height: dh });
    const ctx = ctx2d(placeholder);
    ctx.fillStyle = '#3a1d1d';
    ctx.fillRect(0, 0, dw, dh);
    return cell('FineHash', placeholder, `<span class="badge">error</span> ${err.message}`, { pending: true });
  }
}
...
function featureImage(img) {
  ...
  renderHeroHash(work);
  renderHeroRender(work, dw, dh);
}

function renderHeroHash(work) {
  const box = document.getElementById('hero-hash');
  try {
    const bytes = finehash.encode(work.data, work.width, work.height);
...
function renderHeroRender(work, dw, dh) {
  const canvas = document.getElementById('hero-render');
  canvas.width = dw;
  canvas.height = dh;
  try {
    const bytes = finehash.encode(work.data, work.width, work.height);
    const { pixels, width, height } = finehash.decode(bytes, { width: dw, height: dh });
...
function syntheticSamples() {
  return buildSamples().map(({ name, canvas }) => ({ name, url: canvas.toDataURL('image/png') }));
}

// ---------------------------------------------------------------------------
// Rows

async function addRow(src, label, { prepend = false } = {}) {
  const img = await loadImage(src);
  const originalBytes = await sourceBytes(src);
...
function handleFiles(files) {
  ...
  for (const { name, url } of [...images].reverse()) {
    addRow(url, name, { prepend: true }).catch(console.error);
  }
  if (images[0]) loadImage(images[0].url).then(featureImage).catch(console.error);
}
...
window.addEventListener('load', async () => {
  ...
  if (samples[0]) loadImage(samples[0].url).then(featureImage).catch(console.error);
  for (const { name, url } of samples) {
    await addRow(url, name).catch(console.error);
  }
});
